...
vector<long long> printFirstNegativeInteger(long long int arr[],long long int n, long long int k) {
    ...
    for(;i<n;i++){
        ...
        //1. remove elements which doesnt belong to the window
        while(!q.empty() && q.front()<=i-k){
            q.pop();
        }
        
        //2.removing elements which are of no use
        while(!q.empty() && arr[q.back()]>=0){
            q.pop();
        }
        
        //adding current new element ot the window
        if(arr[i]<0){
            q.push(i);
        }
    }
