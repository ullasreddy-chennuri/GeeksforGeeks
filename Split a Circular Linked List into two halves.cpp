...
The address of the head of the linked list, addresses of the head of the first and second halved resultant lists 
and Set the head1_ref and head2_ref to the first resultant list and second resultant list respectively.
...
void splitList(Node *head, Node **head1_ref, Node **head2_ref)
{
    ...
    // Assinging head for first list
    *head1_ref = head;
    
    // Head for second list
    if(head->next!=head){
       *head2_ref = slow->next;
    }
    
    // circular for second list
    fast->next=slow->next;
    
    // circular for first list
    slow->next=head;
